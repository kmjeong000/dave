...
// Typedefs
#if CGAL_VERSION_MAJOR >= 6
typedef std::optional<cgal::AABBTree::Intersection_and_primitive_id<cgal::Ray>::Type>
  RayIntersection;
#else
typedef boost::optional<cgal::AABBTree::Intersection_and_primitive_id<cgal::Ray>::Type>
  RayIntersection;
#endif
double Geometry::TriangleArea(
  const cgal::Point3 & _p0, const cgal::Point3 & _p1, const cgal::Point3 & _p2)
{
...
double Geometry::TriangleArea(const cgal::Triangle & _tri)
{
...
cgal::Point3 Geometry::TriangleCentroid(const cgal::Triangle & _tri)
{
  return CGAL::centroid(_tri[0], _tri[1], _tri[2]);
}

cgal::Point3 Geometry::MidPoint(const cgal::Point3 & _p0, const cgal::Point3 & _p1)
{
...
cgal::Vector3 Geometry::Normal(const cgal::Triangle & _tri)
{
...
cgal::Point3 Geometry::HorizontalIntercept(
  const cgal::Point3 & _high, const cgal::Point3 & _mid, const cgal::Point3 & _low)
{
  ...
  return cgal::Point3(
    _low.x() + t * (_high.x() - _low.x()), _low.y() + t * (_high.y() - _low.y()), _mid.z());
}

bool Geometry::RayIntersectsTriangle(
  const cgal::Point3 & _origin, const cgal::Direction3 & _direction, const cgal::Triangle & _tri,
...
bool Geometry::LineIntersectsTriangle(
  ...
  cgal::Point3 & _intersection)
{
  return LineIntersectsTriangle(_origin, _direction, _tri[0], _tri[1], _tri[2], _intersection);
}

bool Geometry::LineIntersectsTriangle(
  const cgal::Point3 & _origin, const cgal::Direction3 & _direction, const cgal::Point3 & _p0,
  const cgal::Point3 & _p1, const cgal::Point3 & _p2, cgal::Point3 & _intersection)
{
  cgal::Vector3 _ray(_direction.vector());
  cgal::Vector3 e1 = _p1 - _p0;
  cgal::Vector3 e2 = _p2 - _p0;
...
bool Geometry::SearchMesh(
  const cgal::Mesh & _mesh, const cgal::Point3 & _origin, const cgal::Direction3 & _direction,
  cgal::Point3 & _intersection)
{
  // AABB Tree
...
bool Geometry::SearchMesh(
  const cgal::AABBTree & _tree, const cgal::Point3 & _origin, const cgal::Direction3 & _direction,
  cgal::Point3 & _intersection)
{
  // Query
  cgal::Ray query(_origin, _direction);
