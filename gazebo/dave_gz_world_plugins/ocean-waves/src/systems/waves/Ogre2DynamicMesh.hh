...
/// a dynamic mesh
class GZ_RENDERING_OGRE2_VISIBLE Ogre2DynamicMesh : public Ogre2Geometry
{
  /// \brief Constructor
  /// \param[in] _scene Pointer to scene
public:
  explicit Ogre2DynamicMesh(ScenePtr _scene);

  /// \brief Virtual destructor
public:
  virtual ~Ogre2DynamicMesh();
...
public:
  void AddPoint(
    const gz::math::Vector3d & _pt, const gz::math::Color & _color = gz::math::Color::White);
...
public:
  void SetNormal(unsigned int _index, const gz::math::Vector3d & _normal);

  /// \brief Change the tangent vector of an existing point in the point list
  /// \param[in] _index Index of the point to set
  /// \param[in] _normal Tangent of the point
public:
  void SetTangent(unsigned int _index, const gz::math::Vector3d & _tangent);
...
public:
  void SetUV0(unsigned int _index, const gz::math::Vector2d & _uv0);
...
private:
  void GenerateNormals(
    Ogre::OperationType _opType, const std::vector<gz::math::Vector3d> & _vertices,
    float * _vbuffer);
...
private:
  std::unique_ptr<Ogre2DynamicMeshPrivate> dataPtr;
};
