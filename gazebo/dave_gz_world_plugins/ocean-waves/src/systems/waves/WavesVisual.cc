...
public:
  ...
  public:
    std::string vertexShaderUri;

    /// \brief Path to fragment shader
  public:
    std::string fragmentShaderUri;
  };

  /// \brief Enum to set the method used for updating shader vertices
public:
  enum class MeshDeformationMethod : uint16_t
  {
    /// \internal
...
public:
  static MeshDeformationMethod SetMeshDeformationMethod(const std::string & _str);
...
public:
  void OnWaveMsg(const gz::msgs::Param & _msg);
...
public:
  gz::math::Vector2i tiles_x = {0, 0};

  /// \brief The number of tiles in the y-direction given as an offset range.
  /// Default [lower=0, upper=0].
public:
  gz::math::Vector2i tiles_y = {0, 0};

  /// \brief Material used by the ocean visual
public:
  rendering::MaterialPtr oceanMaterial;

  /// \brief Pointer to ocean visual
public:
  std::vector<rendering::OceanVisualPtr> oceanVisuals;

public:
  std::vector<rendering::VisualPtr> oceanVisuals2;

public:
  std::vector<rendering::OceanGeometryPtr> oceanGeometries;

public:
  MeshDeformationMethod meshDeformationMethod{MeshDeformationMethod::DYNAMIC_GEOMETRY};
...
public:
  gz::common::MeshPtr oceanTileMesh;
...
public:
  bool waveParamsDirty{false};

  /// \brief The ocean tile managing the wave simulation
public:
  waves::visual::OceanTilePtr oceanTile;
...
public:
  std::map<std::string, ShaderUri> shaders;

  /// \brief A list of shader params
public:
  std::vector<ShaderParamValue> shaderParams;

  /// \brief A list of shader params that will be updated every iteration
public:
  std::vector<ShaderParamValue> timeParams;
...
public:
  rendering::RenderEngineExtension * extension{nullptr};
};

/////////////////////////////////////////////////
WavesVisual::WavesVisual() : System(), dataPtr(std::make_unique<WavesVisualPrivate>())
{
  gzmsg << "WavesVisual: constructing\n";
}

/////////////////////////////////////////////////
WavesVisual::~WavesVisual() { gzmsg << "WavesVisual: destructing\n"; }

/////////////////////////////////////////////////
void WavesVisual::Configure(
  ...
  EntityComponentManager & _ecm, EventManager & _eventMgr)
{
  ...
  // Update parameters
  {
    this->dataPtr->isStatic =
      waves::Utilities::SdfParamBool(*sdf, "static", this->dataPtr->isStatic);

    this->dataPtr->tiles_x =
      waves::Utilities::SdfParamVector2i(*sdf, "tiles_x", this->dataPtr->tiles_x);

    this->dataPtr->tiles_y =
      waves::Utilities::SdfParamVector2i(*sdf, "tiles_y", this->dataPtr->tiles_y);

    std::string meshDeformationMethodStr =
      waves::Utilities::SdfParamString(*sdf, "mesh_deformation_method", "DYNAMIC_GEOMETRY");
  ...
  if (sdf->HasElement("param"))
  {
    ...
    while (paramElem)
    {
      if (!paramElem->HasElement("shader") || !paramElem->HasElement("name"))
      {
        gzerr << "<param> must have <shader> and <name> sdf elements" << std::endl;
  ...
  if (this->dataPtr->modelPath.empty())
  {
    ...
    this->dataPtr->modelPath = _ecm.ComponentData<components::SourceFilePath>(modelEntity).value();
  }
  ...
  // Shader programs
  {
    ...
    while (shaderElem)
    {
      if (!shaderElem->HasElement("vertex") || !shaderElem->HasElement("fragment"))
      {
        gzerr << "<shader> must have <vertex> and <fragment> sdf elements" << std::endl;
      }
      else
      {
        // default to glsl
  ...
  this->dataPtr->connection = _eventMgr.Connect<gz::sim::events::SceneUpdate>(
  ...
  this->dataPtr->node.Subscribe(topic, &WavesVisualPrivate::OnWaveMsg, this->dataPtr.get());

  gzmsg << "WavesVisual: subscribing to [" << topic << "]\n";
}

//////////////////////////////////////////////////
void WavesVisual::PreUpdate(const UpdateInfo & _info, EntityComponentManager &)
{
  ...
  this->dataPtr->currentSimTimeSeconds =
    (std::chrono::duration_cast<std::chrono::nanoseconds>(this->dataPtr->currentSimTime).count()) *
    1e-9;
}

/////////////////////////////////////////////////
GZ_ENUM(
  meshDeformationMethodIface, WavesVisualPrivate::MeshDeformationMethod,
  WavesVisualPrivate::MeshDeformationMethod::MESH_DEFORMATION_METHOD_BEGIN,
  WavesVisualPrivate::MeshDeformationMethod::MESH_DEFORMATION_METHOD_END, "UNKNOWN",
  "DYNAMIC_TEXTURE", "DYNAMIC_GEOMETRY")

//////////////////////////////////////////////////
WavesVisualPrivate::MeshDeformationMethod WavesVisualPrivate::SetMeshDeformationMethod(
...
void WavesVisualPrivate::OnUpdate()
{
  ...
  if (!this->extension)
  {
    extension = rendering::RenderEngineExtensionManager::Instance()->Extension("ogre2");
  }
  ...
  switch (this->meshDeformationMethod)
  {
    case MeshDeformationMethod::DYNAMIC_GEOMETRY:
    {
      ...
      if (this->oceanVisuals2.empty() && oceanVisuals.empty())
      {
        ...
        rendering::SceneNodeFactoryPtr sceneNodeFactory = this->extension->SceneNodeFactory();
        ...
        for (waves::Index iy = this->tiles_y[0]; iy <= this->tiles_y[1]; ++iy)
        {
          for (waves::Index ix = this->tiles_x[0]; ix <= this->tiles_x[1]; ++ix)
          {
            ...
            gz::math::Vector3d tilePosition(
              position.X() + ix * lx, position.Y() + iy * ly, position.Z() + 0.0);
...
#define GZ_WAVES_UPDATE_VISUALS 1
      ...
      if (this->waveParamsDirty)
      {
        this->oceanTile->SetWindVelocity(
          this->waveParams->WindVelocity().X(), this->waveParams->WindVelocity().Y());
        this->oceanTile->SetSteepness(this->waveParams->Steepness());
    ...
    case MeshDeformationMethod::DYNAMIC_TEXTURE:
    {
      ...
      if (this->waveParamsDirty)
      {
        this->mWaveSim->SetWindVelocity(
          this->waveParams->WindVelocity().X(), this->waveParams->WindVelocity().Y());
        this->mWaveSim->SetSteepness(this->waveParams->Steepness());
        this->waveParamsDirty = false;
      }
  ...
  for (const auto & spv : this->shaderParams)
  {
    ...
    if (spv.type == "texture")
    {
      unsigned int uvSetIndex =
        spv.args.empty() ? 0u : static_cast<unsigned int>(std::stoul(spv.args[0]));
      std::string texPath = gz::common::findFile(asFullPath(spv.value, this->modelPath));
      (*params)[spv.name].SetTexture(
        texPath, rendering::ShaderParam::ParamType::PARAM_TEXTURE, uvSetIndex);

      gzmsg << "Shader param [" << spv.name << "]"
            << ", type: " << spv.type << ", tex coord set: " << uvSetIndex << "\n";
    }
    else if (spv.type == "texture_cube")
    {
      ...
      std::string texPath = gz::common::findFile(asFullPath(spv.value, this->modelPath));
      (*params)[spv.name].SetTexture(
        texPath, rendering::ShaderParam::ParamType::PARAM_TEXTURE_CUBE, uvSetIndex);
    ...
    else
    {
      ...
      rendering::ShaderParam::ParamType paramType = rendering::ShaderParam::PARAM_NONE;
...
void WavesVisualPrivate::InitTextures()
{
  ...
  this->displacementMap =
    sceneNodeFactory->CreateDisplacementMap(this->scene, this->oceanMaterial, this->entity, nx, ny);
...
void WavesVisualPrivate::UpdateWaveSim()
{
  ...
  mWaveSim->DisplacementAndDerivAt(
    mHeights, mDisplacementsX, mDisplacementsY, mDhdx, mDhdy, mDxdx, mDydy, mDxdy);
}
...
void WavesVisualPrivate::UpdateTextures()
{
  this->displacementMap->UpdateTextures(
    this->mHeights, this->mDhdx, this->mDhdy, this->mDisplacementsX, this->mDisplacementsY,
    this->mDxdx, this->mDydy, this->mDxdy);
}
...
//////////////////////////////////////////////////
GZ_ADD_PLUGIN(
  gz::sim::systems::WavesVisual, gz::sim::System, gz::sim::systems::WavesVisual::ISystemConfigure,
  gz::sim::systems::WavesVisual::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(gz::sim::systems::WavesVisual, "gz::sim::systems::WavesVisual")
