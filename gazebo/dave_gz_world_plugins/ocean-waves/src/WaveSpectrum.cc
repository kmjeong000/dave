...
//////////////////////////////////////////////////
PiersonMoskowitzWaveSpectrum::PiersonMoskowitzWaveSpectrum(double u19, double gravity)
: OmniDirectionalWaveSpectrum(), gravity_(gravity), u19_(u19)
{
}
...
void PiersonMoskowitzWaveSpectrum::Evaluate(
  Eigen::Ref<Eigen::ArrayXXd> spectrum, const Eigen::Ref<const Eigen::ArrayXXd> & k) const
{
  /// \note Eigen asserts cbegin and cend are from the same expression.
  auto k_view = k.reshaped();
  std::transform(
    k_view.cbegin(), k_view.cend(), spectrum.reshaped().begin(),
    [this](double k_i) -> double { return this->Evaluate(k_i); });
}

//////////////////////////////////////////////////
double PiersonMoskowitzWaveSpectrum::Gravity() const { return gravity_; }

//////////////////////////////////////////////////
void PiersonMoskowitzWaveSpectrum::SetGravity(double value) { gravity_ = value; }

//////////////////////////////////////////////////
double PiersonMoskowitzWaveSpectrum::U19() const { return u19_; }

//////////////////////////////////////////////////
void PiersonMoskowitzWaveSpectrum::SetU19(double value) { u19_ = value; }
...
//////////////////////////////////////////////////
ECKVWaveSpectrum::ECKVWaveSpectrum(double u10, double cap_omega_c, double gravity)
: OmniDirectionalWaveSpectrum(), gravity_(gravity), u10_(u10), cap_omega_c_(cap_omega_c)
{
}
...
double ECKVWaveSpectrum::Evaluate(double k) const
{
  ...
  double cap_gamma =
    std::exp(-1.0 / (2.0 * std::pow(sigma, 2.0)) * std::pow(std::sqrt(k / kp) - 1.0, 2.0));

  double j_p = std::pow(gamma, cap_gamma);

  double f_p = l_pm * j_p * std::exp(-0.3162 * cap_omega_c_ * (std::sqrt(k / kp) - 1.0));

  double f_m = l_pm * j_p * std::exp(-0.25 * std::pow(k / km - 1.0, 2.0));
...
//////////////////////////////////////////////////
double ECKVWaveSpectrum::Gravity() const { return gravity_; }

//////////////////////////////////////////////////
void ECKVWaveSpectrum::SetGravity(double value) { gravity_ = value; }

//////////////////////////////////////////////////
double ECKVWaveSpectrum::U10() const { return u10_; }

//////////////////////////////////////////////////
void ECKVWaveSpectrum::SetU10(double value) { u10_ = value; }

//////////////////////////////////////////////////
double ECKVWaveSpectrum::CapOmegaC() const { return cap_omega_c_; }

//////////////////////////////////////////////////
void ECKVWaveSpectrum::SetCapOmegaC(double value) { cap_omega_c_ = value; }
