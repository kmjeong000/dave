...
{
//////////////////////////////////////////////////
LinearRandomFFTWaveSimulationRef::Impl::~Impl() { DestroyFFTWPlans(); }

//////////////////////////////////////////////////
LinearRandomFFTWaveSimulationRef::Impl::Impl(double lx, double ly, Index nx, Index ny)
: lambda_(0.6), lx_(lx), ly_(ly), nx_(nx), ny_(ny)
{
...
//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::Impl::SetSteepness(double value) { lambda_ = value; }
...
//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::Impl::ElevationAt(Eigen::Ref<Eigen::ArrayXXd> h)
{
  // run the FFT
...
void LinearRandomFFTWaveSimulationRef::Impl::ComputeBaseAmplitudes()
{
  ...
  for (Index ikx = 0; ikx < nx_; ++ikx)
  {
    ...
    {
      double k = sqrt(kx_math_[ikx] * kx_math_[ikx] + ky_math_[iky] * ky_math_[iky]);
      ...
      else
      {
        ...
        if (use_symmetric_spreading_fn_)
        {
          ...
          cap_psi = Impl::ECKVSpreadingFunction(k, phi - phi10_, u10_, cap_omega_c_, gravity_);
        }
        else
        {
          // travelling waves - asymmetric spreading function
          cap_psi =
            Impl::Cos2sSpreadingFunction(s_param_, phi - phi10_, u10_, cap_omega_c_, gravity_);
        }
        double cap_s = Impl::ECKVOmniDirectionalSpectrum(k, u10_, cap_omega_c_, gravity_);
...
//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::Impl::ComputeCurrentAmplitudes(double time)
{
  // alias
  ...
  for (Index iky = 1; iky < ny_ / 2 + 1; ++iky)
  {
    Index ikx = 0;
    zhat(ikx, iky) = complex(
      +(r(ikx, iky) * psi_root(ikx, iky) + r(ikx, ny_ - iky) * psi_root(ikx, ny_ - iky)) *
          cos_omega_k(ikx, iky) +
        (s(ikx, iky) * psi_root(ikx, iky) + s(ikx, ny_ - iky) * psi_root(ikx, ny_ - iky)) *
          sin_omega_k(ikx, iky),
      -(r(ikx, iky) * psi_root(ikx, iky) - r(ikx, ny_ - iky) * psi_root(ikx, ny_ - iky)) *
          sin_omega_k(ikx, iky) +
        (s(ikx, iky) * psi_root(ikx, iky) - s(ikx, ny_ - iky) * psi_root(ikx, ny_ - iky)) *
  ...
  for (Index ikx = 1; ikx < nx_ / 2 + 1; ++ikx)
  {
    Index iky = 0;
    zhat(ikx, iky) = complex(
      +(r(ikx, iky) * psi_root(ikx, iky) + r(nx_ - ikx, iky) * psi_root(nx_ - ikx, iky)) *
          cos_omega_k(ikx, iky) +
        (s(ikx, iky) * psi_root(ikx, iky) + s(nx_ - ikx, iky) * psi_root(nx_ - ikx, iky)) *
          sin_omega_k(ikx, iky),
      -(r(ikx, iky) * psi_root(ikx, iky) - r(nx_ - ikx, iky) * psi_root(nx_ - ikx, iky)) *
          sin_omega_k(ikx, iky) +
        (s(ikx, iky) * psi_root(ikx, iky) - s(nx_ - ikx, iky) * psi_root(nx_ - ikx, iky)) *
...
void LinearRandomFFTWaveSimulationRef::Impl::CreateFFTWPlans()
{
  ...
  // elevation
  fft_plan0_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_.data()),
    reinterpret_cast<fftw_complex *>(fft_out0_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan1_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_ikx_.data()),
    reinterpret_cast<fftw_complex *>(fft_out1_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan2_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_iky_.data()),
    reinterpret_cast<fftw_complex *>(fft_out2_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);

  // xy-displacements
  fft_plan3_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_sx_.data()),
    reinterpret_cast<fftw_complex *>(fft_out3_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan4_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_sy_.data()),
    reinterpret_cast<fftw_complex *>(fft_out4_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan5_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_kxkx_.data()),
    reinterpret_cast<fftw_complex *>(fft_out5_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan6_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_kyky_.data()),
    reinterpret_cast<fftw_complex *>(fft_out6_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
  fft_plan7_ = fftw_plan_dft_2d(
    nx_, ny_, reinterpret_cast<fftw_complex *>(fft_h_kxky_.data()),
    reinterpret_cast<fftw_complex *>(fft_out7_.data()), FFTW_BACKWARD, FFTW_ESTIMATE);
}
  ...
  double k, double phi, double u10, double cap_omega_c, double gravity)
{
  ...
  double cap_phi =
    (1 + tanh(ao + ap * pow(c / cp, 2.5) + am * pow(cm / c, 2.5)) * cos(2.0 * phi)) / 2.0 / M_PI;
  return cap_phi;
}
...
//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::SetLambda(double value) { impl_->SetLambda(value); }
...
//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::SetSteepness(double value) { impl_->SetSteepness(value); }

//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::SetTime(double value) { impl_->SetTime(value); }

//////////////////////////////////////////////////
Index LinearRandomFFTWaveSimulationRef::SizeX() const { return impl_->nx_; }

//////////////////////////////////////////////////
Index LinearRandomFFTWaveSimulationRef::SizeY() const { return impl_->ny_; }

//////////////////////////////////////////////////
Index LinearRandomFFTWaveSimulationRef::SizeZ() const { return 0; }

//////////////////////////////////////////////////
void LinearRandomFFTWaveSimulationRef::ElevationAt(Eigen::Ref<Eigen::ArrayXXd> h) const
{
  impl_->ElevationAt(h);
}
