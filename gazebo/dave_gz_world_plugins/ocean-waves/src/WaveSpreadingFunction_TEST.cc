...
TEST(WaveSpreadingFunction, Cos2sRegression)
{
  {  // Regress against values generate from Python reference version
    ...
    double theta[]{0.0,        0.31415927, 0.62831853, 0.9424778,  1.25663706, 1.57079633,
    ...
    double phi[] = {9.03278127e-01, 7.05050192e-01, 3.31091480e-01, 8.98336768e-02, 1.30308992e-02,
                    8.82107546e-04, 2.18874516e-05, 1.24955907e-07, 5.69467280e-11, 6.95777405e-17,
                    0.00000000e+00, 6.95777405e-17, 5.69467280e-11, 1.24955907e-07, 2.18874516e-05,
                    8.82107546e-04, 1.30308992e-02, 8.98336768e-02, 3.31091480e-01, 7.05050192e-01,
                    9.03278127e-01};
...
TEST(WaveSpreadingFunction, Cos2sVectorXd)
{
  {  // Eigen array version
    ...
    theta << 0.0, 0.31415927, 0.62831853, 0.9424778, 1.25663706, 1.57079633, 1.88495559, 2.19911486,
      2.51327412, 2.82743339, 3.14159265, 3.45575192, 3.76991118, 4.08407045, 4.39822972,
      4.71238898, 5.02654825, 5.34070751, 5.65486678, 5.96902604, 6.28318531;
...
TEST(WaveSpreadingFunction, Cos2sVirtualVectorXd)
{
  {  // Call virtually from base class ptr.
    ...
    std::unique_ptr<DirectionalSpreadingFunction> spreadingFn = std::move(derivedSpreadingFn);
...
TEST(WaveSpreadingFunction, Cos2sFFT2ImplRegression)
{
  ...
  {  // Eigen array version
    ...
    for (Index i = 0; i < 21; ++i)
    {
      ...
      double phi_test = LinearRandomFFTWaveSimulationRef::Impl::Cos2sSpreadingFunction(
...
TEST(WaveSpreadingFunction, WaveNumberArrayXXd)
{
  {  // componentwise operations
    ...
    Eigen::ArrayXXd theta = ky.binaryExpr(kx, [](double y, double x) { return std::atan2(y, x); });
...
TEST(WaveSpreadingFunction, ECKVRegression)
{
  {  // Regress against values generate from Python reference version
    ...
    double phi[] = {0.26371613, 0.24374672, 0.19146613, 0.12684376, 0.07456316, 0.05459375,
...
TEST(WaveSpreadingFunction, ECKVArrayXXd)
{
  {  // Eigen array version
    ...
    for (Index i = 0; i < nx; ++i)
    {
      for (Index j = 0; j < ny; ++j)
      {
        double phi_test = spreadingFn.Evaluate(theta(i, j), theta_mean, k(i, j));
...
TEST(WaveSpreadingFunction, ECKVFFT2ImplRegression)
{
  ...
  {  // Eigen array version
    ...
    {
      double dtheta = theta(i) - theta_mean;
      double phi_test = LinearRandomFFTWaveSimulationRef::Impl::ECKVSpreadingFunction(
