...
void TriangulatedGrid::Private::CreateTriangulation()
{
  ...
  for (auto f = indices_.begin(); f != indices_.end(); ++f, ++idx)
  {
    ...
    cgal::Point3 p((p0.x() + p1.x() + p2.x()) / 3.0, (p0.y() + p1.y() + p2.y()) / 3.0, 0.0);
    fh = tri_.locate(p, fh);
...
bool TriangulatedGrid::Private::Height(const cgal::Point3 & query, double & height) const
{
  ...
  if (fh != nullptr)
  {
    ...
    found = Geometry::LineIntersectsTriangle(query, direction, triangle, intersection);
...
bool TriangulatedGrid::Private::Height(
  const std::vector<cgal::Point3> & queries, std::vector<double> & heights) const
{
  bool foundAll = true;
...
//////////////////////////////////////////////////
const Point3Range & TriangulatedGrid::Private::Points() const { return points_; }

//////////////////////////////////////////////////
const Index3Range & TriangulatedGrid::Private::Indices() const { return indices_; }

//////////////////////////////////////////////////
const cgal::Point3 & TriangulatedGrid::Private::Origin() const { return origin_; }
...
void TriangulatedGrid::Private::ApplyPose(const gz::math::Pose3d & pose)
{
  ...
  for (auto && it = std::make_pair(std::begin(points0_), std::begin(points_));
       it.first != std::end(points0_) && it.second != std::end(points_); ++it.first, ++it.second)
  {
...
void TriangulatedGrid::Private::DebugPrintTriangulation() const
{
  ...
  {
    std::cout << "vertex: " << idx << ", mesh vertex: " << v->info() << "\n";
  }
  ...
  {
    std::cout << "face: " << idx << ", mesh face: " << f->info() << "\n";
  }
}
...
//////////////////////////////////////////////////
void TriangulatedGrid::CreateMesh() { impl_->CreateMesh(); }

//////////////////////////////////////////////////
void TriangulatedGrid::CreateTriangulation() { impl_->CreateTriangulation(); }

//////////////////////////////////////////////////
std::unique_ptr<TriangulatedGrid> TriangulatedGrid::Create(Index nx, Index ny, double lx, double ly)
{
  std::unique_ptr<TriangulatedGrid> instance = std::make_unique<TriangulatedGrid>(nx, ny, lx, ly);
  instance->CreateMesh();
...
//////////////////////////////////////////////////
const Point3Range & TriangulatedGrid::Points() const { return impl_->Points(); }

//////////////////////////////////////////////////
const Index3Range & TriangulatedGrid::Indices() const { return impl_->Indices(); }

//////////////////////////////////////////////////
const cgal::Point3 & TriangulatedGrid::Origin() const { return impl_->Origin(); }

//////////////////////////////////////////////////
void TriangulatedGrid::ApplyPose(const gz::math::Pose3d & pose) { impl_->ApplyPose(pose); }

//////////////////////////////////////////////////
bool TriangulatedGrid::IsValid(bool verbose) const { return impl_->IsValid(verbose); }

//////////////////////////////////////////////////
void TriangulatedGrid::DebugPrintMesh() const { impl_->DebugPrintMesh(); }

//////////////////////////////////////////////////
void TriangulatedGrid::DebugPrintTriangulation() const { impl_->DebugPrintTriangulation(); }
...
//////////////////////////////////////////////////
void TriangulatedGrid::UpdatePoints(const cgal::Mesh & from) { impl_->UpdatePoints(from); }

//////////////////////////////////////////////////
std::array<double, 2> TriangulatedGrid::TileSize() const { return {impl_->lx_, impl_->ly_}; }

//////////////////////////////////////////////////
std::array<Index, 2> TriangulatedGrid::CellCount() const { return {impl_->nx_, impl_->ny_}; }
