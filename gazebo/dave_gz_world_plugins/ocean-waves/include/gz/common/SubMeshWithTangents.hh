...
public:
  explicit SubMeshWithTangents(const std::string & _name);

  /// \brief Copy constructor
  /// \param[in] _submesh SubMeshWithTangents to copy.
public:
  SubMeshWithTangents(const SubMeshWithTangents & _submesh);

  /// \brief Move constructor
  /// \param[in] _collision SubMeshWithTangents to move.
public:
  SubMeshWithTangents(SubMeshWithTangents && _submesh) noexcept;

  /// \brief Copy assignment operator.
  /// \param[in] _submesh SubMeshWithTangents to copy.
  /// \return Reference to this.
public:
  SubMeshWithTangents & operator=(const SubMeshWithTangents & _submesh);

  /// \brief Move assignment operator.
  /// \param[in] _collision Collision component to move.
  /// \return Reference to this.
public:
  SubMeshWithTangents & operator=(SubMeshWithTangents && _submesh) noexcept;

  /// \brief Destructor
public:
  virtual ~SubMeshWithTangents();

  /// \brief Add a tangent to the mesh
  /// \param[in] _n The tangent
public:
  void AddTangent(const gz::math::Vector3d & _tangent);
...
public:
  void AddTangent(const double _x, const double _y, const double _z);
...
public:
  gz::math::Vector3d Tangent(const unsigned int _index) const;

  /// \brief Set a tangent
  /// \param[in] _index Index of the tangent that will be set.
  /// \param[in] _n The new tangent direction
public:
  void SetTangent(const unsigned int _index, const gz::math::Vector3d & _tangent);

  /// \brief Return the number of tangents
  /// \return The number of tangents.
public:
  unsigned int TangentCount() const;
  ...
  /// _index.
public:
  bool HasTangent(const unsigned int _index) const;
